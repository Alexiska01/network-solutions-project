        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filteredModels.map((model, idx) => (
              <div key={idx} className="bg-white rounded-xl shadow-lg border border-gray-100 p-8 hover:shadow-xl transition duration-300 hover:-translate-y-1">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">{model.name}</h3>
                <p className="text-gray-600 mb-4">{model.desc}</p>
                <Button className="w-full bg-brand-primary hover:bg-gradient-hero text-white font-medium transition duration-300" onClick={() => (window.location.href = model.link)}>

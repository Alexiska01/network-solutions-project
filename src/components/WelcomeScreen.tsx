...
const StarField3D: React.FC = () => {
  const stars = useMemo(() => {
    // Минимальное количество звезд на мобильных для максимальной производительности
    const starCount = typeof window !== 'undefined' && window.innerWidth < 768 ? 30 : 300;
...
const CosmicObjects: React.FC = () => {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {/* Астероидное поле - минимальное на мобильных */}
      {Array.from({ length: typeof window !== 'undefined' && window.innerWidth < 768 ? 2 : 12 }, (_, i) => (
        <motion.div
          key={i}
...
const Satellite3D: React.FC<{ progress: number }> = ({ progress }) => {
  ...
  return (
    <div className="relative w-16 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 lg:w-40 lg:h-40 mx-auto mb-4 sm:mb-6 md:mb-8">
      ...
      {/* Вращающийся спутник - оптимизированный */}
      <motion.div

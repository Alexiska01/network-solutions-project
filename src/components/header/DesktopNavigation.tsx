...
const DesktopNavigation = ({
  ...
  return (
    <nav className="hidden lg:flex items-center space-x-4 mx-0">
      {navigationItems.map((item) => (
        <div key={item.path} className="relative">
          {item.hasSubmenu ? (
            ...
            <Link
              to={item.path}
              className="text-gray-700 hover:text-blue-600 px-2 py-2 text-sm font-medium transition-colors whitespace-nowrap flex items-center space-x-2 h-10"
